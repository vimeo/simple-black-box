...
set_http_probe () {
        ...
        sudo ngrep -W byline $http_pattern > $output/http &
        internal=1 assert_num_procs "^ngrep.*$http_pattern" 1
}

# $1 res_match: egrep-compatible expression to match http responses codes, example: '(200|201)'
assert_all_http_responses () {
        ...
        local num_match=$(egrep -c "^HTTP/1\.. $res_match" $output/http)
        num_all=$(egrep -c "^HTTP/1\.. " $output/http)
        if [ $num_match -ne $num_all ]; then
                fail "only $num_match http response code(s) matching '$res_match' out of $num_all total"
                egrep "^HTTP/1\.. " $output/http | debug_stream "all http response codes:"
        else
...
assert_num_http_requests () {
        ...
        num_match_req=$(egrep -A 1 "$regex_socket_info" $output/http | egrep -v "$regex_socket_info" | grep -v ^HTTP | grep -v '^\-\-' | egrep -c "$match_req")
        egrep -A 1 "$regex_socket_info" $output/http | egrep -v "$regex_socket_info" | grep -v ^HTTP | grep -v '^\-\-' | egrep "$match_req" | debug_stream "http requests matching '$match_req'"
