...
set_udp_statsd_probe () {
	local ref=$1
        local pattern="$2"
        [ -n "$ref" ] || die_error "set_udp_statsd_probe () needs an instance reference string"
        [ -n "$pattern" ] || die_error "set_udp_statsd_probe () needs a non-zero pattern to match the traffic"
        debug "set_udp_statsd_probe $ref '$pattern'"
        sudo tcpdump -tttt -n -A $pattern 2>&1 | egrep -v '^(tcpdump|listening on|$|[^ ]+ packets)' > $output/udp-statsd_$ref &
...
assert_num_udp_statsd_requests () {
	local ref=$1
        ...
        [ -n "$ref" ] || die_error "assert_num_udp_statsd_requests () needs an instance reference string"
        ...
        num_match_req=$(grep -c "$match_req" $output/udp-statsd_$ref)
        grep "$match_req" $output/udp-statsd_$ref | debug_stream "udp_statsd requests matching '$match_req'"
        if [ $num_match_req -ge $match_req_min ]; then
                if [ "$match_req_max" == "I" ] || [ $num_match_req -le $match_req_max ]; then
                        win "udp_statsd_$ref: $num_match_req request(s) matching '$match_req', which is between $match_req_min (min) and $match_req_max (max)"
                        return
                fi
        fi
        fail "udp_statsd_$ref: $num_match_req request(s) matching '$match_req' which is not between $match_req_min (min) and $match_req_max (max)"
}
