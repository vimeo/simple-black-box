...
test_post () {
        echo "Error: failed to check for existence of container $container" >> $stderr
        ...
        assert_http_response_to "^PUT" 202 1
       ...
       # assert_pattern "container.*$container.*already existing" 1 $stdout $stderr $log
        assert_pattern "created.*$container" 0 $stdout $stderr $log
